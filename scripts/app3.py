...
# ... (other functions like load_data, load_model, etc.)
...
def main():
    ...
    # --- THE FIX: CSS for the floating DIV ---
    st.markdown("""
    <style>
    /* Base styles (kept for reference) */
    .big-font { font-size:20px !important; font-weight: bold; }
    .result-box p { color: black; }

    /* CRITICAL FIX: The new fixed floating button style */
    .chatbot-floating-btn {
        ...
        /* Appearance matching the request */
    ...
    # --- NAVIGATION FIX: Python logic to read URL and update state ---
    # This must run before the sidebar radio is displayed.
    if 'main_nav_radio' not in st.session_state:
        st.session_state['main_nav_radio'] = "Salary Prediction"
        
    try:
        # Check for the custom query parameter set by the HTML onclick
        query_params = st.experimental_get_query_params()
        if 'nav_to' in query_params:
            nav_page = query_params['nav_to'][0]
            if nav_page in ["Salary Prediction", "Career Growth Projection", "Top Paying Roles", "Real-Time Salary Check", "AI Assistant (Chatbot)"]:
                st.session_state['main_nav_radio'] = nav_page
                
                # IMPORTANT: Clear the query parameter to prevent the selection from sticking
                # This causes an extra rerun, but ensures clean URL
                st.experimental_set_query_params(nav_to=None)

    except Exception as e:
        # Handle cases where experimental APIs might be restricted or changed
        # This is a safe fallback, but usually unnecessary
        pass
        
    # --- FLOATING CHATBOT ICON IMPLEMENTATION ---
    # Use the custom DIV with JS onclick event to set the query parameter
    st.markdown(f"""
    <div class="chatbot-floating-btn" 
        title="Go to AI Assistant (Chatbot)"
        onclick="window.location.href = window.location.pathname + '?nav_to=AI%20Assistant%20(Chatbot)';">
        🤖
    </div>
    """, unsafe_allow_html=True)
    # --- END FLOATING CHATBOT ICON ---
    ...
    # The radio button will now reflect the value set in st.session_state
...
def render_chatbot(df):
    ...
    st.markdown("<div id='ai-assistant-chatbot'></div>", unsafe_allow_html=True)
...
def render_salary_prediction(df, model, column_order):
    ...
    # ... (rest of render_salary_prediction remains the same)
    st.subheader("Skills")
...
def render_top_paying_roles(df):
    ...
    fig = px.bar(
        ...
        orientation='h', 
        title=chart_title, 
        labels={'salary': 'Average Salary', 'job_title': 'Job Title'}, 
        color='salary', 
